...
if (Meteor.isServer) {
  describe('notes', function () {
    ...
    it('should not update note with extra fields', function() {
      const title = 'This is an updated title';
      const extra = 'This is extra';

      expect(() => {
        Meteor.server.method_handlers['notes.update'].apply({userId: noteOne.userId},
          [noteOne._id, {title, extra}]);
      }).toThrow();
    });
    ...
    it('should not update note if user was not creator', function() {
      ...
      Meteor.server.method_handlers['notes.update'].apply({userId: 'notUser'},
          [noteOne._id, {title}]);
    ...
    it('should return no notes for user that has none', function() {
      const res = Meteor.server.publish_handlers.notes.apply({userId: 'notUser'});
      const notes = res.fetch();
